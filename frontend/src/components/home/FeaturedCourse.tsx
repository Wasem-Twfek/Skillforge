...
import { Clock, Users, Star, ChevronRight, BookOpen, Award, LockKeyhole } from 'lucide-react';
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="p-6 sm:p-8">
        <p className="mb-8 text-gray-700 dark:text-gray-300">{featuredCourse.description}</p>

        <div className="grid gap-6 sm:grid-cols-2">
          <div>
            <h4 className="mb-4 flex items-center text-lg font-semibold text-gray-900 dark:text-white">
              <BookOpen className="mr-2 h-5 w-5 text-primary-500" />Course content
            </h4>
            <div className="space-y-3">
              {featuredCourse.lessons.slice(0, 3).map((lesson, index) => (
                <div key={lesson.id} className="flex items-center gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-100 text-sm font-medium dark:bg-gray-700">{index + 1}</span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-medium text-gray-900 dark:text-white">{lesson.title}</p>
                    {lesson.duration !== undefined && <p className="text-sm text-gray-500 dark:text-gray-400">{lesson.duration} minutes</p>}
                  </div>
          ...
          <div>
            <h4 className="mb-4 flex items-center text-lg font-semibold text-gray-900 dark:text-white">
              <Award className="mr-2 h-5 w-5 text-primary-500" />Included
            </h4>
            <ul className="space-y-3 text-gray-700 dark:text-gray-300">
              <li>{featuredCourse.lessons.length} lessons</li>
        ...
        <Link to={'/courses/' + featuredCourse.id} className="mt-8 inline-flex w-full items-center justify-center rounded-xl bg-primary-600 px-6 py-3.5 font-medium text-white transition hover:bg-primary-700">
          Open Course
